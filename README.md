...
| 动作 | 频率 | 要网络 | 耗时 |
|---|---|---|---|
| **本地提交** `git commit` | **每次改完就做** | ❌ | 1 秒 |
| **推送** `git push` | **攒起来统一推** | ✅ | 失败时很慢 |

> **原因：** 国内网络到 github.com 时通时不通，每次改完就推会反复失败、浪费时间。
> 所以本地提交照常做，推送攒到一批再统一做。
...
`## main...origin/main [ahead 6]` 里的数字就是待推送的提交数。
